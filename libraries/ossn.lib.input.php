...
function input($input, $noencode = '', $default = false, $strip = true) {
		...
		if($hook) {
				...
				if(!isset($hook['data']) && $default !== false) {
						return $default;
				}

				if(isset($hook['data'])) {
						if($noencode === '' || $noencode === false) {
								$str = htmlspecialchars($hook['data'], ENT_QUOTES, 'UTF-8');
				...
				if(strlen($str) > 0) {
						...
						$str = ossn_input_escape($str);
						return $strip ? trim($str) : $str;
				}
		}
