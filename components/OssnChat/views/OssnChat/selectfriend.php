...
?>
       ...
       <!-- $arsalan.shah tab container start -->    
        <div class="tab-container"> 
           ...
           <div class="ossn-chat-tab-titles" id="ftab-t<?php echo $user->guid;?>" onclick="Ossn.ChatCloseTab(<?php echo $user->guid;?>);">
                <div class="text ossn-chat-inline-table"><?php echo $user->fullname;?></div>
                <div class="options ossn-chat-inline-table">
                   <div class="ossn-chat-inline-table ossn-chat-icon-expend" title="Popout Chat" onclick="Ossn.ChatExpand('<?php echo $user->username;?>')"></div> 
                  <div class="ossn-chat-inline-table ossn-chat-tab-close" id="ftab-c<?php echo $user->guid;?>" onclick="Ossn.ChatTerminateTab(<?php echo $user->guid;?>);"> X</div>
                </div>
           </div>
         ...
         <div class="data" id="ossn-chat-messages-data-<?php echo $user->guid;?>">
                 <?php
		 foreach(ossn_chat()->get(ossn_loggedin_user()->guid, $user->guid) as $message){
         ...
         </div>   <!-- $arsalan.shah datatend -->
        
        </div>        <!-- $arsalan.shah tab container end -->    
        <div class="inner friend-tab <?php echo $tab_class;?>" id="ftab<?php echo $user->guid;?>" onclick="Ossn.ChatOpenTab(<?php echo $user->guid;?>);">  
             ...
             <form autocomplete="off" id="ossn-chat-send-<?php echo $user->guid;?>">
               ...
               <div class="ossn-chat-inline-table ossn-chat-icon-smile-set">
                    <div class="ossn-chat-inline-table ossn-chat-icon-smile" onClick="Ossn.ChatShowSmilies(<?php echo $user->guid;?>);" style="margin-top: 5px;"></div>
               </div>
