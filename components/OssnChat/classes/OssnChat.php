...
class OssnChat extends OssnDatabase {
   ...
   public function countOnlineFriends($user, $intervals = 100){
	   $online = get_object_vars($this->getOnlineFriends($user, $intervals));
	   return count($online);
   }
    /**
	...
	*/     
    public static function setUserChatSession($to){
	    ...
	    if(!in_array($to, $_SESSION['ossn_chat_users'])){ 		
	...
	*/    
    public function getNewAll($parm = array()){
		if(empty($params)){
		 $parm = array('message_from', 'message');	
		}
		$parm = implode(',' , $parm);
	 	$user = ossn_loggedin_user()->guid;
		$params['from'] = 'ossn_messages';
		$params['params'] = array("{$parm}");
	    $params['wheres'] = array("message_to='{$user}' AND viewed='0'");  
	    $friends = $this->select($params, true);			  
        return $friends;
    }   
    /**
	...
	*/     
    public static function removeChatTab($tab){
	  if(isset($_SESSION['ossn_chat_users']) 
						 && in_array($tab, $_SESSION['ossn_chat_users'])){  		 
