		...
		public function searchObject(array $params = array()) {
				...
				if($options['search_type'] === true) {
						if(!empty($options['title'])) {
								$wheres[] = OssnDatabase::wheres('o.title', 'LIKE', "%{$options['title']}%");
						}
						if(!empty($options['description'])) {
								$wheres[] = OssnDatabase::wheres('o.description', 'LIKE', "%{$options['description']}%");
						}
				} elseif($options['search_type'] === false) {
						if(!empty($options['title'])) {
								$wheres[] = OssnDatabase::wheres('o.title', '=', $options['title']);
						}
						if(!empty($options['description'])) {
								$wheres[] = OssnDatabase::wheres('o.description', '=', $options['description']);
						}
				}
