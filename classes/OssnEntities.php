 ...
 */	
 public function add(){
	 ...
	 if(!empty($this->owner_guid) && in_array($this->type, $this->entity_types)){
	     ...
	     $this->params['names'] = array('owner_guid', 'type', 'subtype', 
								        'time_created', 'time_updated', 
								        'premission', 'active'
								       );
	     $this->params['values'] = array(
								         $this->owner_guid, $this->type, $this->subtype,
								         $this->time_created, $this->time_updated,
								         $this->premission, $this->active
								        );
	     if($this->insert($this->params)){
			 $this->params['into'] = 'ossn_entities_metadata';
	         $this->params['names'] = array('guid', 'value');
	         $this->params['values'] = array($this->getLastEntry(), $this->value);
 ...
 */ 
 public function get_entities(){
	  ...
	  foreach($this->get as $entites){
	    ...
	    unset($metadata->id);
		$data = array_merge(
							  get_object_vars($entites),
							  get_object_vars($metadata)
							  );
